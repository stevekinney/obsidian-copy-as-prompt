 ...
 *
 * The clipboard holds one payload at a time, and a `ClipboardItem` carries at
 * most one image, so "text plus three screenshots" cannot be a single copy.
 * Two separate operations solve it: text goes through the ordinary web API,
 * and images go on as *file references* via the macOS pasteboard, so one paste
 * into a chat window attaches all of them at once.
 */
